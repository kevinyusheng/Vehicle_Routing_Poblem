...
def validate(num_nodes,data_loader, actor, reward_fn, render_fn=None, save_dir='.',
             num_plot=5):
    ...
    for batch_idx, batch in enumerate(data_loader):
        if batch_idx != 1:
            continue
        static, dynamic, x0 , correspond , render_record = batch         ###correspond
        static = static.to(device)
        dynamic = dynamic.to(device)
        x0 = x0.to(device) if len(x0) > 0 else None
                   
        with torch.no_grad():
            tour_indices, _ = actor.forward(static, dynamic,correspond,render_record, x0)    ###correspond

        reward = reward_fn(num_nodes,correspond,static, tour_indices).mean().item()
        ...
        if render_fn is not None and batch_idx < num_plot:
            ...
            print(path)
            render_fn(render_record,static, tour_indices, path)
...
def train(actor, critic, task, num_nodes, train_data, valid_data, reward_fn,
    ...
    for epoch in range(2):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            static, dynamic, x0 , correspond, render_record = batch
            ...
            tour_indices, tour_logp = actor(static, dynamic,correspond,render_record , x0)        ###correspond

            # Sum the log probabilities for each city in the tour
            reward = reward_fn(num_nodes,correspond,static, tour_indices)
        ...
        mean_valid = validate(num_nodes,valid_loader, actor, reward_fn, render_fn,
...
def train_vrp(args):
    ...
    LOAD_DICT = {10: 20, 20: 40, 50: 80, 100: 120}
    MAX_DEMAND = 9
    STATIC_SIZE = 2 # (x, y)
    ...
    test_dir = 'test'
    test_loader = DataLoader(test_data, args.batch_size, False, num_workers=0)
    out = validate(args.num_nodes,test_loader, actor, vrp.reward, vrp.render, test_dir, num_plot=5)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed', default=11059, type=int)
    ...
    args.checkpoint = os.path.join('vrp', '20', '12_17_19.123809' + os.path.sep)
